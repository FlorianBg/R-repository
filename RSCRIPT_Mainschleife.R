...
# Further Information on the Study: https://github.com/FlorianBg/R-repository/blob/master/README.md


#     >> what is done: <<
# Analysis of water depth measurement data to extract dates of very low and medium water levels.
# Download and processing of Sentinel 2 imagery corresponding to the water depth
# classification of water surface to compare low water values to mean- or medium-values,
# in order to search for difference in horizontal expanse of the river surface.

# also some explorative plotting is executed during the analysis; final plots provided by last section


#     >> input data: << 
# - Water level depth measurements:           https://github.com/FlorianBg/R-repository/blob/master/Data.zip
# - Sentinel 2 imagery have to be downloaded via code in script
# - polygon training areas:                   https://github.com/FlorianBg/R-repository/blob/master/Data.zip
...
# (explore water level records to find Sentinel 2 images in next section, which represent the horizontal
# expanse of water surface at corresponding water depths)

  # read Waterlevel measurements of the Main river from Astheim:
...
plot(Mainschleife) #                                                    see how the study area looks
crs(Mainschleife) #                                                     check out the CRS
...
# 5) ANALYSIS / CLASSIFICATION AND ACCURACY ASSESSMENT
  ...
  # test an unsupervised classification RStoolbox to explore the data
...
td_low #                      inspect and view training areas: with the classes:: 'water', 'urban', 'field', 'forest', 'ground'
names(td_low)
View(td_low)
plot(td_low)
...
SClow <- raster("D:/Dokumente/R_working_directory/Classification_low.tif")
...
SCmean <- raster("D:/Dokumente/R_working_directory/Classification_mean.tif")
...
plot(NDVI_low, 
     main = "NDVI", 
     sub = "19.09.2018; 80cm  depth",
     cex.axis = 0.6,
     xlab = "UTM Westing Coordinate (m)",
     ylab = "UTM Northing Coordinate (m)")
plot(NDVI_mean, 
     main = "NDVI", 
     sub = "19.07.2018; 102cm depth",
     cex.axis = 0.6,
     xlab = "UTM Westing Coordinate (m)",
     ylab = "UTM Northing Coordinate (m)")
dev.off()
...
plot(td_mean, 
     col = "blue", 
     main = "Training areas mean level",
     add = TRUE)
dev.off()
...
plot(changeAnalysis, 
     ...
     sub = "-1 : not flooded at mean depths but flooded at low depths, 
     +1 : flooded at mean depths but not flooded at low depths",
     xlab = "UTM Westing Coordinate (m)",
     ylab = "UTM Northing Coordinate (m)")
dev.off()
